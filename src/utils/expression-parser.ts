...
export function postHydrationFormatting(expression: string, fields: FormField[], fieldValues: Record<string, any>) {
  const matchedFields = [];
  let hydratedExpression = expression;
  ...
  if (matchedFields?.length) {
    matchedFields.forEach((field) => {
      hydratedExpression = replaceFieldRefWithValuePath(field, fieldValues[field.id], expression);
    });
  }
